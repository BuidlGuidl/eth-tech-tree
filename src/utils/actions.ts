import fs from "fs";
import { execa } from "execa";
import inquirer from "inquirer";
import { loadUserState } from "./stateManager";
import { submitChallengeToServer } from "../modules/api";

export async function testChallenge(name: string) {
    console.log("Running tests on challenge...");
    const targetDir = `challenges/${name}`;
    // Check if challenge exists
    if (!fs.existsSync(targetDir)) {
        console.log("Challenge does not exist. Please setup the challenge first.");
        return;
    }
    // Install dependencies and run tests
    const { failed: installFailed } = await execa("yarn", ["install"], { cwd: targetDir });
    if (installFailed) {
        console.log("Failed to install dependencies.");
        return;
    }

    try {
        await execa("yarn", ["run", "foundry:test"], { cwd: targetDir, all: true }).pipeAll!(process.stdout);
    } catch (error) {
        console.log("");
        console.log("Challenge does not pass tests. Examine the output above and try again when you have fixed the issues.");
        return;
    }
    console.log("");
    console.log("Challenge passed the tests! Great work! Now deploy your contract.");
}

export async function submitChallenge(name: string) {
    const { address: userAddress } = loadUserState();
    // Prompt the user for the contract address
    const questions = [
        {
            type: "input",
            name: "address",
            message: "Completed challenge contract address on Sepolia:",
            validate: (value: string) => /^0x[a-fA-F0-9]{40}$/.test(value),
        },
    ];
    const answers = await inquirer.prompt(questions);
    const { address } = answers;
    console.log("Submitting challenge...");
    // Send the contract address to the server
    const response = await submitChallengeToServer(userAddress as string, "sepolia", name, address);
    if (response.passed) {
        console.log("Challenge submitted successfully!");
    } else {
        console.log(response.error);
    }
}